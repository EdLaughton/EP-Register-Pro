...
const { deriveProceduralPosture } = require('./epo_v2_posture_signals');
...
function deriveCurrentPosture({ main = {}, docs = [], legal = {} } = {}) {
  ...
  const legalEvents = Array.isArray(legal.events) ? legal.events : [];
  const toRecord = (entry = {}, source = 'Unknown') => ({
    dateStr: entry.dateStr || '',
    title: entry.title || '',
    detail: entry.procedure || entry.detail || '',
    actor: entry.actor || (/applicant|filed by applicant|by applicant/i.test(`${entry.title || ''} ${entry.detail || ''}`) ? 'Applicant' : 'EPO'),
    source,
    codexKey: entry.codexKey || '',
    codexPhase: entry.codexPhase || '',
    codexClass: entry.codexClass || '',
  });
  const uniqueRecords = [];
  const seen = new Set();
  for (const record of [
    ...docs.map((doc) => toRecord(doc, 'Documents')),
    ...legalEvents.map((event) => toRecord(event, 'Event')),
    ...codedEvents.map((event) => toRecord(event, 'Coded legal event')),
  ]) {
    const key = [record.dateStr, record.title, record.detail, record.source, record.codexKey].join('|');
    if (seen.has(key)) continue;
    seen.add(key);
    uniqueRecords.push(record);
  }

  const posture = deriveProceduralPosture({ statusRaw, records: uniqueRecords });
  const signals = {
