...
const unitaryRenewal = hooks.inferRenewalModel({ filingDate: '10.10.2019' }, { events: [], renewals: [{ dateStr: '13.09.2023', year: 5 }] }, { ueStatus: 'Unitary effect registered' }, { renewalFeesPaidUntil: 'Year 17' });
...
assert.strictEqual(unitaryRenewal.nextYear, 18, 'Renewal model should use the federated UP paid-through year when projecting the next unitary renewal');
assert(/Year 18/.test(unitaryRenewalCard), 'Renewal card should render the next unitary renewal from the federated paid-through horizon rather than a stale earlier legal-renewal year');
assert(/federated register reports payments through Year 17/i.test(unitaryRenewalCard), 'Renewal card should acknowledge when federated UP renewal data outruns the raw legal-event payment history');
